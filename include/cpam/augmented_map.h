...
public:
  ...
  static M aug_filter(M m, const F& f) {
    auto x = Tree::aug_filter(m.get_root(), f);    
    return M(x); 
  }
  ...
  // template<class F>
  // static M aug_filter2(M m, const F& f) {
  //   return M(Tree::aug_filter2(m.root, f)); }

  //  mid-order traversal filter added.
  template<class F>
  ...
  A aug_val2(node* t) const {return Tree::aug_val(t); }
  ...
  template<class Func, class Fbb>
  static M filter_bb(M m, const Func &fpt, const Fbb &fbb){
    return to_aug(Map::filter_bb(std::move(m), fpt, fbb));
  }
  ...
  static M multi_delete(M m, parlay::sequence<E> const &SS) { //  added interface for multi delete
  ...
  template <class outItter>
  static void values(M m, outItter out) {Map::values(std::move(m), out); }  // values added

  static parlay::sequence<V> values(M m, size_t granularity=kNodeLimit) {
	  return Map::values(m, granularity);
  }
