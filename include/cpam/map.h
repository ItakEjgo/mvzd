...
public:
  ...
  static parlay::sequence<V> values(M m, size_t granularity=kNodeLimit) {
    auto f = [] (E e) -> V {return Entry::get_val(e);};
    return to_seq<V>(std::move(m), f, granularity);
  }
  ...
  static M filter_bb(M m, const F& f, const F2 &f2, size_t granularity=kNodeLimit) {
    return M(Tree::finalize(Tree::filter_bb(m.get_root(), f, f2, granularity))); }
...
//  }
  ...
  static M multi_delete(M m, Seq const &SS) {
    ...
    parlay::sequence<K> B(A.size());
    parlay::parallel_for(0, A.size(), [&](int i){
      B[i] = get<0>(A[i]);
    });
    t.next("build time");
...
//    t.next("union time");
    auto x = M(Tree::multi_delete_sorted(m.get_root(), B.data(), B.size()));
    t.next("MI time");
    return x;
  }
