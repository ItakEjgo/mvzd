...
//      Tree::check_structure(ret);
  ...
  static size_t size_in_bytes(node* a, const F& f) {
    ...
    if (Tree::is_compressed(a)) {
      ...
      // std::cout << "leaf pnt, size = " << pnt_cnt << ", " << c->size_in_bytes << std::endl;
    } else {
...
//      return to_tree_impl(arr, arr_size);
  ...
  static node* filter_bb(ptr b, const Func& f, const Fbb &fb, size_t granularity=kNodeLimit) {
    if (b.empty()) return NULL;
    size_t n = b.size();
...
//    }
    auto cur_aug = sequence_ops::aug_val(b.unsafe_ptr());
    if (!fb(cur_aug.first)) return NULL;
