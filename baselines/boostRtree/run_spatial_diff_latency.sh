...
echo "[INFO] Dealing Small Region Query:"
numactl --membind=0 --physcpubind=0 ./main -i $Unifile -a combined -t spatial-diff -r $Uniqry0 > output/boost-100M-U-0.log
numactl --membind=0 --physcpubind=0 ./main -i $Varfile -a combined -t spatial-diff -r $Varqry0 > output/boost-100M-V-0.log

echo "[INFO] Dealing Median Region Query:"
numactl --membind=0 --physcpubind=0 ./main -i $Unifile -a combined -t spatial-diff -r $Uniqry1 > output/boost-100M-U-1.log
numactl --membind=0 --physcpubind=0 ./main -i $Varfile -a combined -t spatial-diff -r $Varqry1 > output/boost-100M-V-1.log
# done
