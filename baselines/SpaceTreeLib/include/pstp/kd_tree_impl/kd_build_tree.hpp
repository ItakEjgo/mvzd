#ifndef PSTP_KD_TREE_IMPL_KD_BUILD_TREE_HPP_
#define PSTP_KD_TREE_IMPL_KD_BUILD_TREE_HPP_
...
Node* KdTree<Point, SplitRule, kSkHeight, kImbaRatio>::SerialBuildRecursive(
  ...
  if (n == 0) {
    // return nullptr; //  test create leaf time
    // [[maybe_unused]] auto cur = AllocEmptyLeafNode<Slice, Leaf>();
    return AllocEmptyLeafNode<Slice, Leaf>();
  }

  if (n <= BT::kLeaveWrap) {
    // return nullptr; //  test create leaf time
    // [[maybe_unused]] auto cur = AllocNormalLeafNode<Slice, Leaf>(In);
  ...
  if (!split.has_value()) {  // split fails
    if (In.end() == std::ranges::find_if_not(In, [&](Point const& p) {
      ...
      // return nullptr; //  test create leaf time
      // [[maybe_unused]] auto cur = AllocDummyLeafNode<Slice, Leaf>(In);
...
Node* KdTree<Point, SplitRule, kSkHeight, kImbaRatio>::BuildRecursive(
  ...
  for (BucketType i = 0; i < BT::kBucketNum; ++i) {
    if (!sums[i]) {
      // [[maybe_unused]] auto cur = AllocEmptyLeafNode<Slice, Leaf>(); //  test build leaf
...
#endif  // PSTP_KD_TREE_IMPL_KD_BUILD_TREE_HPP_
