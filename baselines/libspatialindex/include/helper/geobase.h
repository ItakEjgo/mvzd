...
namespace geobase
{
    ...
    template <typename T>
    auto merge_pts(parlay::sequence<Point> &lhs, T &rhs)
    {
        size_t i = 0, j = 0;
        ...
        auto added = parlay::sequence<Point>::uninitialized(rhs.size());
        auto removed = parlay::sequence<Point>::uninitialized(lhs.size());
        ...
        while (i < lhs.size() && j < rhs.size())
        {
            ...
            { // first smaller, in A not in B
                parlay::assign_uninitialized(removed[cnt_remove++], lhs[i++]);
            }
            else
            { //  second smaller, in B not in A
                parlay::assign_uninitialized(added[cnt_add++], rhs[j++]);
            }
        }
        while (i < lhs.size())
        {
