...
echo "[INFO] Dealing Small Region Query:"
numactl --membind=0 --physcpubind=0 ./test -i $Unifile -a combined -t spatial-diff -r $Uniqry0 > output/100M-U-0.log
numactl --membind=0 --physcpubind=0 ./test -i $Varfile -a combined -t spatial-diff -r $Varqry0 > output/100M-V-0.log

echo "[INFO] Dealing Median Region Query:"
numactl --membind=0 --physcpubind=0 ./test -i $Unifile -a combined -t spatial-diff -r $Uniqry1 > output/100M-U-1.log
numactl --membind=0 --physcpubind=0 ./test -i $Varfile -a combined -t spatial-diff -r $Varqry1 > output/100M-V-1.log
# done
